                            ...
                            @if($cart->count() > 0)
                                ...
                                <table>
                                    ...
                                    @foreach($cart->getItems() as $producto)
                                        <tr>
                                            <td>
                                                {{ Form::open(array('name' => 'formElimina_' . $producto->id, 'url' => 'arriendo/carrito/' . $producto->id)) }}
                                            ...
                                            <td style="text-align:left">
                                                ...
                                                <a href="{{ URL::to('/arriendo/ficha/' . $producto->id . '/' . Str::slug($producto->nombre, '-')) }}.html" >{{$producto->nombre}}</a>
                                            </td>
                                            <td>
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        <div class="row text-right">
                                            <div style="padding-right: 35px;" class="col-md-6 col-md-offset-6">
                                                <a href="{{URL::to('/arriendo/checkout')}}" class="btn btn-mega">Confirmar Solicitud</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
@section('javascript')
    <script>
        ...
        function disminuirProducto(elemento) {
            ...
            $.ajax({
                type: "POST",
                url: '{{URL::to('arriendo/disminuirproducto')}}',
        ...
        function incrementarProducto(elemento) {
            ...
            $.ajax({
                type: "POST",
                url: '{{URL::to('arriendo/incrementarproducto')}}',
                data: {'id': id, 'cantidad': cantidad, 'unidad' : unidad, '_token' : token},
                success: function(response) {
                    var respuesta = JSON.parse(response);
                    $(elemento).val(cantidad).parents('tr').find('span[rel="valor"]').html('$'+$.formatNumber(respuesta.totalitem));
